...
static THD_FUNCTION(Thread1, arg) {
    ...
    error_t e;
    e = eddsaGenerateKeyPair(YARROW_PRNG_ALGO, &prngcontext, ED25519_CURVE, &privatekey, &publickey);
    ...
    for (i = 0; i < sizeof(available_ms_table); i++){
      ...
      while (loops<100) {
          ...
          ed25519GenerateSignature((uint8_t*)(&privatekey)->d.data, (uint8_t*)(&publickey)->q.data, message,
                                   sizeof(message), &contextsign, sizeof(&contextsign), 0,
                                   signature);
...
//          chMtxUnlock(&mutex);
          ...
          signature_verified = ed25519VerifySignature((uint8_t*)(&publickey)->q.data, message,
          sizeof(message), &contextsign, sizeof(&contextsign), 0,
          signature);
