   ...
   size_t k, uint_t e, RsaPrivateKey *privateKey)
{
   ...
   do
   {
      do
      {
         ...
         // Define your large prime number (replace these values)
         uint_t largePrimeValue[32] = {
             0x2EE9D95B, 0x7F3B1A0C, 0xE0D6B7A3, 0x4BB5D135,
             0xEB33C6D4, 0x617BE4A5, 0xA607A93A, 0x911C19E6,
             0x1FFDAE61, 0xBBD18906, 0x1528F8C0, 0x4F53FE95,
             0x2EBF5407, 0x8A1C31A8, 0x3AB59D85, 0x6C408AB1,
             0xC616CC4E, 0x8DA97C4B, 0xD4E80157, 0xF6B3A79E,
             0x6A6ECF00, 0xCBE21568, 0x4A2D97EE, 0x8886852A,
             0x3F18CAB1, 0x83A1BC0B, 0x9F09E1AC, 0x5AB6B3C3,
             0x4604D02D, 0xF5528FA6, 0x24CC3B8B, 0x7DDBACF0
         };

         privateKey->p.data = &largePrimeValue;

         //Test whether p is a probable prime
         uint_t i = 0;
         error = NO_ERROR; //mpiCheckProbablePrime(&privateKey->p);
//         do{
//           error = NO_ERROR; //mpiCheckProbablePrime(&privateKey->p);
//           i++;
//           mpiAddInt(&privateKey->p, &privateKey->p, 2);
//         } while(error == ERROR_INVALID_VALUE && i < 1000);
   ...
   do
   {
      do
      {
         ...
         uint_t primeData[32] = {
             0x71A32F87, 0x52CB950F, 0x70B67C76, 0xFEC31E7B,
             0x59DAD16D, 0x03222FDF, 0x5CE7F917, 0x7A7FED9D,
             0xD875ED13, 0xB7B9E4AC, 0xB102D5B1, 0x7EF446A7,
             0xC6DEA06B, 0x99AE9D79, 0x39FBC9A9, 0xAE671D4D,
             0xC938C3C8, 0xF32082F8, 0x51F8ABE0, 0xB3EB4A4A,
             0x7F9D65F0, 0xA3CFFA02, 0x5486A987, 0x0805E6A9,
             0x97931BDF, 0x717799E4, 0xDE9B4BC7, 0x96D19B16,
             0x9C3D9027, 0x1F25A83D, 0x53C1F127, 0x50643D14
         };
         privateKey->q.data = &primeData;
         //Test whether q is a probable prime
         error = NO_ERROR; //mpiCheckProbablePrime(&privateKey->q);
